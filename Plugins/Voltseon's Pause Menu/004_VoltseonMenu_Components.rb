...
class PokemonPartyHud < Component
  def startComponent(viewport)
	...
	@shadowbmp  = Bitmap.new(MENU_FILE_PATH + "overlayShadow")
  end
  ...
  def refresh
    ...
    for i in 0...$Trainer.party.length
      ...
      if pokemon.shadowPokemon?
        @sprites["overlay"].bitmap.blt(spacing + (Graphics.width/8) + 52, Graphics.height/2 - 142,
                          @shadowbmp,Rect.new(0, 0, @shadowbmp.width, @shadowbmp.height))
      end
      # Item Icon
      if pokemon.hasItem?
...
class DateAndTimeHud < Component
  ...
  def refresh
    text = _INTL("STR:{1} INT:{2}", $game_variables[76], $game_variables[14])
	text2 = _INTL("CHR: {1} SKL:{2}", $game_variables[17], $game_variables[16])
    text3 = _INTL("NRG: {1} INTER:{2}", $game_variables[18], $game_variables[30])
    @sprites["overlay"].bitmap.clear
    pbSetSystemFont(@sprites["overlay"].bitmap)
    pbDrawTextPositions(@sprites["overlay"].bitmap,[[text,Graphics.width/2 - 8, 0,1,
