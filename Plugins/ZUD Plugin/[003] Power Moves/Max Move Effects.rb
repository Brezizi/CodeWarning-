...
# G-Max Aurastorm, G-Max Soulshards, G-Max Aura Vortex, G-Max Soulburn, G-Max Hellfire.
#===============================================================================
...
class PokeBattle_Move_D002 < PokeBattle_MaxMove_StatUp
  def initialize(battle,move,newMove)
    super
    @statUp = [:ATTACK,1]           if @id == :GMAXAURASTORM
    @statUp = [:ATTACK,1]           if @id == :GMAXAURAVORTEX
    @statUp = [:ATTACK,1]           if @id == :GMAXSOULBURN
    @statUp = [:ATTACK,6]           if @id == :GMAXSOULCRUSH
    @statUp = [:SPEED,6]            if @id == :GMAXSOULBREAK
    @statUp = [:SPEED,6]            if @id == :GMAXSOULVORTEX
  end 
end
...
class PokeBattle_Move_D003 < PokeBattle_MaxMove_TargetStatDown
  def initialize(battle,move,newMove)
    ...
    @statDown = [:ATTACK,1]          if @id == :MAXSTEELSPIKE
    @statDown = [:DEFENSE,1]         if @id == :MAXPHANTASM
    @statDown = [:DEFENSE,1]         if @id == :MAXKNUCKLE
    @statDown = [:SPECIAL_ATTACK,1]  if @id == :MAXFLUTTERBY
    @statDown = [:SPECIAL_ATTACK,1]  if @id == :MAXQUAKE
    @statDown = [:SPECIAL_DEFENSE,1] if @id == :MAXDARKNESS
    @statDown = [:SPECIAL_DEFENSE,1] if @id == :MAXOOZE
    @statDown = [:SPEED,1]           if @id == :MAXSTRIKE
    @statDown = [:SPEED,1]           if @id == :MAXAIRSTREAM
...
#===============================================================================
# Max Flare, Max Geyser, Max Hailstorm, Max Rockfall.
#===============================================================================
...
class PokeBattle_Move_D004 < PokeBattle_MaxMove_Weather
  def initialize(battle,move,newMove)
    ...
    @weatherType = :Snow      if @id == :MAXHAILSTORM
...
class PokeBattle_Move_D006 < PokeBattle_MaxMove
  def pbEffectGeneral(user)
    ...
    if @id == :GMAXARMAGEDDON && user.pbOpposingSide.effects[PBEffects::Wildfire]==0
      user.pbOpposingSide.effects[PBEffects::Wildfire]=4
      @battle.pbDisplay(_INTL("{1} became surrounded by Exo's flames!",user.pbOpposingTeam))
    end
  end
end
...
#===============================================================================
# G-Max Wind Rage, G-Max Aura Vortex.
#===============================================================================
...
#===============================================================================
# G-Max Resonance, G-Max Soulrend, G-Max Soulshards.
#===============================================================================
