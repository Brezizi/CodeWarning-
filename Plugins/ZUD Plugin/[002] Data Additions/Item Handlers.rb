...
ItemHandlers::UseInField.add(:MAXEGGS,proc { |item|
  ...
  pbFadeOutIn {
    ...
    for i in $Trainer.pokemon_party
      next if i.level>=pbLevelCap ||  i.egg?
      gainers    = true
      maxexp     = i.growth_rate.minimum_exp_for_level(pbLevelCap + 1) - 1
      ...
      scene.pbDisplay(_INTL("{1} gained {2} Exp. Points!",i.name,experience.to_s_formatted))
      leveldif.times do
        pbSEPlay("Pkmn move learnt")
