...
class Pokemon
  ...
  def dynamaxAble?
    return false if egg? || species_data.no_dynamax
    return true
  end
  ...
  def calc_stats
    ...
    if self.shiny? && $game_variables[19]>$game_variables[20] && !self.shadowPokemon?|| self.shadowPokemon? && $game_variables[20]>$game_variables[19] &&  !self.shiny?
		
      @attack = (stats[:ATTACK] * 1.05).floor 
	  @defense = (stats[:DEFENSE] * 1.05).floor 
      @spatk = (stats[:SPECIAL_ATTACK] *1.05).floor
      @spdef = (stats[:SPECIAL_DEFENSE] *1.05).floor
      elsif self.shiny? && $game_variables[20]>$game_variables[19] && !self.shadowPokemon?|| self.shadowPokemon? && $game_variables[19]>$game_variables[20] && !self.shiny?
      @attack = (stats[:ATTACK] /1.05).floor 
	  @defense = (stats[:DEFENSE] / 1.05).floor 
      @spatk = (stats[:SPECIAL_ATTACK] *1.05).floor
      @spdef = (stats[:SPECIAL_DEFENSE] /1.05).floor
      elsif self.shiny? && $game_variables[20]==$game_variables[19] &&self.shadowPokemon?
      @attack = (stats[:ATTACK] *1.15).floor 
	  @defense =(stats[:DEFENSE] * 1.15).floor 
      @spatk   = (stats[:SPECIAL_ATTACK] *1.15).floor
      @spdef   = (stats[:SPECIAL_DEFENSE] *1.15).floor
	  else
	  @attack = (stats[:ATTACK] * 1.05).floor 
	  @defense = (stats[:DEFENSE] * 1.05).floor 
      @spatk = (stats[:SPECIAL_ATTACK] *1.05).floor
